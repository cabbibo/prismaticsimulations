...
void main(){
  ...
  vec4 vel    =  texture2D( t_oPos , position.xy );
  ...
  vNorm = normalize(pos.xyz-vel.xyz);//vView;

  vec3 left = normalize(cross(dir, vec3(0.,0.,1.)));

  vec3 fPos = pos.xyz - left * 100.  * lineWidth* (position.z -.5);
  ...
  vec3 eye = normalize( fPos.xyz - (modelViewMatrix * vec4( 0.,0.,0. , 1.0 )).xyz );
  vec3 norm = normalize(normalMatrix * vNorm);
  
  vSEM = semLookup( eye , norm );


  vColor = spectral(vID.y + idInTail * .1 - .3 + vUv.x);
